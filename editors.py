...
def parameter_form(fields, title="Parameters", message=None):
    """
    ADa simple parameter form.
    ...
    returns : list[dict] | None
        Returns updated fields on OK, None on cancel.
    """
    ...
    from System.Windows.Controls import (  # type: ignore
        ...
        RowDefinition, ColumnDefinition, TextBox, Orientation
    )
    ...
    title_tb = TextBlock()
    title_tb.Text = str(title or "Parameters")
    ...
    if message:
        msg_tb = TextBlock()
        msg_tb.Text = str(message)
        msg_tb.Foreground = SolidColorBrush(FG_LIGHT)
        msg_tb.FontSize = 13
        msg_tb.Margin = Thickness(0, 0, 0, 12)
        body_stack.Children.Add(msg_tb)
    ...
    table = Grid()
    table.ColumnDefinitions.Add(ColumnDefinition())  # Label
    table.ColumnDefinitions.Add(ColumnDefinition())  # Value
    ...
    for row_idx, field in enumerate(data_fields):
        ...
        tb_label = TextBlock()
        tb_label.Text = label
        tb_label.Foreground = SolidColorBrush(FG_LIGHT)
        tb_label.FontSize = 13
        tb_label.Margin = Thickness(4, 4, 12, 4)
        tb_label.VerticalAlignment = VerticalAlignment.Center
        Grid.SetRow(tb_label, row_idx)
        Grid.SetColumn(tb_label, 0)
        table.Children.Add(tb_label)

        editor = TextBox()
        editor.Text = value
        editor.Height = 26
        editor.Margin = Thickness(4, 2, 4, 4)
    ...
    def _ok(sender, args):
        for field, editor in editors:
            field["value"] = str(getattr(editor, "Text", ""))
        result["ok"] = True
        win.Close()
