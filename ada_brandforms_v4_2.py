...
# ada_brandforms_v4_2.py — branded WPF dialogs for pyRevit (CPython3)
# Defaults: maxw=1100 and position="center"
# - Autosize + width clamp so long messages wrap (no oversize windows)
# - DPI-safe layout (Auto bottom row), min button sizes
# - Center on Revit window by default; opt-in top-center
# - Clean event wiring, consistent indentation
# - Fallback to pyRevit forms, then WinForms shim
__brandforms_version__ = "v4.2"
...
if _WPF:
    ADABLUE = _hex_to_color(BRAND["BLUE"])
    ADAPINK = _hex_to_color(BRAND["PINK"])
    FGCOLOR = _hex_to_color(BRAND["FG"])
    ...
    def _set_owner_and_position(win, position="center", top_offset=24):
        """Center relative to Revit main window by default; 'top-center' available."""
        try:
    ...
    class BrandForms(object):
        ...
        def alert(message, title="Message", yes=False, no=False, ok=False, cancel=False, *, autosize=True,
            ...
            elif ok or not any([yes, no, cancel]):
                btns = [("OK", "ok")]
            else:
                btns = [("Cancel", "cancel")]
            ...
            """.format(title=title, message=message, size_attr=size_attr, blue=BRAND["BLUE"],
                       pink=BRAND["PINK"], bg=BRAND["BG"], fg=BRAND["FG"], maxw=int(maxw))

            win = XamlReader.Parse(xaml)
            _chrome(win)
            _set_owner_and_position(win, position, top_offset)
            ...
            win.ShowDialog()
            if result[0] in ("yes", "ok"):
                return True
            if result[0] in ("no", "cancel"):
                return False
            return None
        ...
        def ask_for_string(prompt="", default=None, title="Input", *, autosize=True,
            ...
            okb = win.FindName("OkBtn")
            cb = win.FindName("CancelBtn")
            box = win.FindName("InputBox")
            okb.Foreground = SolidColorBrush(FGCOLOR)
            cb.Foreground = SolidColorBrush(FGCOLOR)
            okb.Background = SolidColorBrush(ADABLUE)
            cb.Background = SolidColorBrush(ADAPINK)
            result = [None]
            def _ok(s, e):
                result[0] = box.Text
                win.Close()
        ...
        class SelectFromList(object):
            ...
            def show(options, title="Select", button_name="OK", multiselect=False, width=760, height=520, *, autosize=False,
                ...
                if not options:
                    return None
                size_attr = _size_attrs(autosize, minw, minh, maxw, maxh,
                                        width if not autosize else None,
                                        height if not autosize else None)
                ...
                result = [None]
                def _ok(s, e):
                    if lb.SelectionMode.ToString() == "Single":
                        result[0] = lb.SelectedItem
                    else:
                        result[0] = [i for i in lb.SelectedItems]
                    win.Close()
                def _cancel(s, e):
                    result[0] = None
                    win.Close()
    ...
    try:
        ...
        def _taskdialog(msg, title="Message", buttons="OK"):
            ...
            mapbtn = {
                ...
                "OKCancel": TaskDialogCommonButtons.Ok | TaskDialogCommonButtons.Cancel,
                "YesNo": TaskDialogCommonButtons.Yes | TaskDialogCommonButtons.No,
            ...
            res = td.Show()
            return {
                TaskDialogResult.Ok: "ok",
                TaskDialogResult.Close: "ok",
                TaskDialogResult.Cancel: "cancel",
                TaskDialogResult.Yes: "yes",
                TaskDialogResult.No: "no",
            }.get(res)
        ...
        class _Shim(object):
            ...
            def alert(message, title="Message", yes=False, no=False, ok=False, cancel=False):
                if yes and no:
                    r = _taskdialog(message, title, "YesNo")
                    return True if r == "yes" else False if r == "no" else None
                if ok and cancel:
                    r = _taskdialog(message, title, "OKCancel")
                    return True if r == "ok" else False if r == "cancel" else None
                if ok or not any([yes, no, cancel]):
                    _taskdialog(message, title, "OK")
                    return True
                if cancel and not ok:
                    r = _taskdialog(message, title, "OKCancel")
                    return False if r == "cancel" else True
            ...
            def ask_for_string(prompt="", default=None, title="Input"):
                ...
                l = Label()
                l.Text = prompt
                l.SetBounds(10, 10, 520, 40)
                f.Controls.Add(l)
                t = TextBox()
                t.Text = default or ""
                t.SetBounds(10, 55, 520, 24)
                f.Controls.Add(t)
                ok = Button()
                ok.Text = "OK"
                ok.SetBounds(360, 150, 80, 24)
                ok.DialogResult = DialogResult.OK
                f.Controls.Add(ok)
                ca = Button()
                ca.Text = "Cancel"
                ca.SetBounds(450, 150, 80, 24)
            ...
            class SelectFromList(object):
                ...
                def show(options, title="Select", button_name="OK", multiselect=False, width=640, height=420):
                    ...
                    lb.SetBounds(10, 10, width - 30, height - 80)
                    for it in options:
                        lb.Items.Add(it)
                    f.Controls.Add(lb)
                    ok = Button()
                    ok.Text = button_name
                    ok.SetBounds(width - 200, height - 60, 80, 24)
                    ok.DialogResult = DialogResult.OK
                    f.Controls.Add(ok)
                    ca = Button()
                    ca.Text = "Cancel"
                    ca.SetBounds(width - 110, height - 60, 80, 24)
                    ca.DialogResult = DialogResult.Cancel
                    f.Controls.Add(ca)
